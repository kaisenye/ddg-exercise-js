...
class SimplicialComplexOperators {
        ...
        assignElementIndices(mesh) {
                ...
                let faceIndex = indexElements(mesh.faces);
        }
        ...
        buildVertexEdgeAdjacencyMatrix(mesh) {
                ...
                let numnberRows = mesh.edges.length;
                let numberColumns = mesh.vertices.length;
        ...
        buildEdgeFaceAdjacencyMatrix(mesh) {
                ...
                let numnberRows = mesh.faces.length;
                let numberColumns = mesh.edges.length;
                let tripletList = new Triplet(numnberRows, numberColumns);
        ...
        buildVertexVector(subset) {
                ...
                let vect = DenseMatrix.zeros(this.mesh.vertices.length, 1);
                subset.vertices.forEach( it => {
                        vect[it] = 1;
                })
        ...
        buildEdgeVector(subset) {
                // TODO
                let vect = DenseMatrix.zeros(this.mesh.edges.length, 1);
                subset.edges.forEach( it => {
                        vect[it] = 1;
                })
        ...
        buildFaceVector(subset) {
                // TODO
                let vect = DenseMatrix.zeros(this.mesh.faces.length, 1);
                subset.faces.forEach( it => {
                        vect[it] = 1;
                })
        ...
        star(subset) {
                ...
                // let star = new MeshSubset(subset.vertices, subset.edges, subset.faces);

                let star = subset;

                const v_vect = this.buildVertexVector(star);
                const extra_edge = this.A0 * v_vect;
                ...
                const e_vect = this.buildEdgeVector(star);
                const extra_face = this.A1 * e_vect;
